...
- **Crop:** free or 1:1, 16:9, 9:16, 4:3 and 3:4 selection; pixel dimensions; Reset / Apply.
  Images use the macOS save sheet: PNG stays PNG; other images become JPEG with a white transparency background.
  MP4/MOV/M4V video previews use the same crop interface and write `-cropped` beside the original.
...
- **Archive:** selection (including folders) becomes `Archive.zip` beside the first input.
  Duplicate basenames are numbered; symlinks remain symlinks; originals stay untouched.
- **Compress:** no window. PNG scanlines are recompressed without altering pixels, metadata or bit depth;
  JPEG coefficients are optimized losslessly with jpegtran. Integer PCM audio up to 24-bit can become FLAC.
...
disabled; only system frameworks/libraries are linked. libjpeg-turbo 3.1.3 provides jpegtran.
