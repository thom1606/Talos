...
final class ModuleLibraryModel {
    ...
    func load() async {
        ...
        do {
            ...
            #if TALOS_SETTINGS
            for index in snapshot.installed.indices where snapshot.installed[index].sourceID == nil {
    ...
    /// Adds Talos's first-party collection on first launch. A missing network
    /// connection (or an unpublished repository) is intentionally silent:
    /// users can still open Settings and it will be retried on the next launch.
    func installDefaultActionsIfNeeded() async {
        ...
        defer { isBusy = false }
        do {
            let existing = snapshot.repositories.firstIndex {
                $0.owner.caseInsensitiveCompare("thom1606") == .orderedSame &&
                    $0.repository.caseInsensitiveCompare("Talos-Actions") == .orderedSame
            }
            var source: RepositorySource
            if let existing {
                source = snapshot.repositories[existing]
            } else {
                source = try RepositorySource(url: Self.talosActionsURL)
            }
            source = try await github.refresh(source, token: nil)
            if let existing {
                snapshot.repositories[existing] = source
            } else {
                snapshot.repositories.append(source)
            }
            try await save()
            // The public source is useful before its first signed release:
            // link it now, and install its actions automatically once release
            // metadata is available.
            guard source.modules.allSatisfy({ $0.release != nil }) else { return }
    ...
    var paletteActions: [PaletteAction] {
        actionItems.map { PaletteAction(id: $0.id, title: $0.action.title, symbol: $0.action.symbol) }
            + [PaletteAction(id: Self.settingsActionID, title: "Settings", symbol: "gearshape")]
    }
    ...
    private func resolve(_ entries: [WheelEntry], files: [ModuleFile]) -> [TalosAction] {
        paginate(entries.compactMap { entry in
            ...
            if entry.actionID == Self.settingsActionID {
                return TalosAction(id: entry.id, title: "Settings", symbol: "gearshape", destination: .handler { [weak self] _ in
                    self?.openSettings()
                })
            }
            ...
            return map(action, module: item.module, placementID: entry.id)
        })
    }

    private func map(_ action: ModuleAction, module: InstalledModule, placementID: String) -> TalosAction {
        let destination: TalosAction.Destination
        if let children = action.children {
            destination = .submenu(paginate(children.map { map($0, module: module, placementID: placementID) }))
        } else {
            destination = .handler { [weak self] urls in
                Task {
                    ...
                    await self.tasks.run(module: module, action: action.id, files: files)
                }
            }
        }
        return TalosAction(id: "\(placementID)/\(action.id)", title: action.title, symbol: action.symbol, destination: destination)
    }
    private func paginate(_ items: [TalosAction]) -> [TalosAction] {
        guard items.count > 8 else { return items }
        return Array(items.prefix(7)) + [TalosAction(id: "more.\(items.count)", title: "More", symbol: "ellipsis", destination: .submenu(paginate(Array(items.dropFirst(7)))))]
    }
