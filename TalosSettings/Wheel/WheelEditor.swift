...
struct WheelEditor: View {
    ...
    @State private var editingEntry: WheelEntry?
    ...
    private func title(_ entry: WheelEntry) -> String {
        entry.children != nil ? entry.title : entry.customTitle ?? model.paletteActions.first { $0.id == entry.actionID }?.title ?? String(localized: "Unavailable action")
    }
    ...
    var body: some View {
        GeometryReader { _ in
            VStack(spacing: 0) {
                ...
                HStack(spacing: 10) {
                    ...
                    .contextMenu {
                        Button("Add to wheel", systemImage: "plus.circle") {
                            Task { await model.setWheelEntries(entries + [.folder(String(localized: "Folder"))], at: folderPath) }
                        }
                ...
                RadialWheel(state: preview, tracksPointer: draggedID == nil, editingActions: true, ghostID: draggedID, removeAction: remove,
                            ...
                            editAction: { id in editingEntry = entries.first { $0.id == id } },
        ...
        .sheet(item: $editingEntry) { entry in
            TileSettingsSheet(model: model, entry: entry)
        }
    ...
    private func openFolder(_ id: String) {
        guard entries.contains(where: { $0.id == id && $0.children != nil }) else { return }
    ...
    private func updateDrag(_ value: DragGesture.Value) {
        ...
        if draggedID == nil {
            ...
            if paletteFrame.contains(start), let item = paletteFrames.first(where: { $0.value.contains(start) }) {
                draggedEntry = item.key == folderPaletteID ? .folder(String(localized: "Folder")) : .action(item.key)
